...
class ChatListScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    if (userStore.currentUser) {
      ...
      if (userStore.justCreated) {
        nav.reset("Preferences", { afterLogin: true });
      }
  ...
  inviteUser = async () => {
    ...
    if (userStore.branchObj) {
      ...
      let { url } = await userStore.branchObj.generateShortUrl(
        linkProperties,
        {}
      );

      Share.open({
        message: I18n.t("invites.download.inviteUser", { url }),
        url: imageToShare,
