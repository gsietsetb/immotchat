...
class NavigationStore {
  ...
  @action
  setNavigator(navigator: NavOrHasRouter) {
    this.navigator = navigator;
    this.navigationState = navigator.router.getStateForAction(
  ...
  dispatchNavigation(
    ...
    const previousNavState: ?NavigationState = reset
      ? null
      : this.navigationState;
    if (this.router) {
      const newState = this.router.getStateForAction(
        routerAction,
        previousNavState
      );
  ...
  @computed
  get state(): NavigationState | (NavigationRoute & NavigationState) {
    return getCurrentState(this.navigationState);
  }

  @autobind
  goBack(key?: ?string): ?NavigationState {
    const navKey: ?string = this.navigationState
      ? this.navigationState.key
      : key;
  ...
  @autobind
  setParams(params: NavigationParams): ?NavigationState {
    return this.dispatchNavigation(
